...
class _PlatformGameList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final showUsage = Localizations.localeOf(
      context,
    ...
    final delegate = SliverChildBuilderDelegate((context, index) {
      ...
      subtitle = _Subtitle(subtitle: text, count: game.usage!.length - 1);
      return Theme(
    ...
    return MultiSliver(
      ...
      children: [
        SliverPersistentHeader(
          pinned: true,
          delegate: _SliverAnimatedPersistentHeader(title: title, asset: asset),
        ),
...
class _Subtitle extends StatelessWidget {
  const _Subtitle({Key? key, required this.subtitle, this.count})
    : super(key: key);
